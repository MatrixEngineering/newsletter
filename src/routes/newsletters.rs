...
fn basic_authorization(header: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    ...
    let password = credentials
        .next()
        .ok_or_else(|| anyhow::anyhow!("A password must be provided in 'Basic' autho."))?
        .to_string();
...
fn verify_password_hash(
    ...
    password_credentiale: Secret<String>,
) -> Result<(), PublishError> {
    ...
    Argon2::default()
        .verify_password(
            password_credentiale.expose_secret().as_bytes(),
